...
function toResolutionVector(width, height) {
  return new Color(width, height, width / Math.max(1, height));
}
...
export default function Galaxy({
  ...
  const focalVector = useMemo(() => new Float32Array(focal), [focal]);
  const rotationVector = useMemo(() => new Float32Array(rotation), [rotation]);
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uRotation: { value: rotationVector },
    ...
    const resize = () => {
      ...
      const width = Math.max(1, Math.floor(container.offsetWidth * pixelRatio));
      const height = Math.max(1, Math.floor(container.offsetHeight * pixelRatio));
      renderer.setSize(width, height);
      program.uniforms.uResolution.value = toResolutionVector(width, height);
    };
